...
switch(dirpath1){
 ...
 if(req.url =='/list/'){
    ...
    if(req.method == "POST"){
      ...
      if(userList[0].username === req.body.username && userList[0].password === req.body.pwd) {
             res.end('/list/',{data:users.chapterList});
   ...
   if(req.method == "POST"){
     ...
     return;
       ...
       res.end(data);
  console.log("我是req.url",req.url);
  console.log("我是method",req.method);
 } else if(req.url == "/add"){
   console.log("fffff");
    var data = fs.readFileSync("./addChapter.html","utf-8");
    res.writeHead(200,{"Content-type":"text/html;charset=UTF-8"});
   ...
   var data = fs.readFileSync("./list.html","utf-8");
    res.writeHead(200,{"Content-type":"text/html;charset=UTF-8"});
      res.end(data);
 ...
 else if(req.url == "/list/listmm"){
    var data = fs.readFileSync("./list.html","utf-8");
    res.writeHead(200,{"Content-type":"text/html;charset=UTF-8"});
    //res.end(data);
    
    if(req.method == 'GET'){
      var uu = JSON.stringify(item);
 ...
 else if(req.url.substring(0,9) == "/list/get"){
   ...
   // console.log("11111111111",cc);
    ...
    console.log("3333333333333333",req.url);
        //res.end(data);     
    if(cc.pathname == '/list/getDetail' && req.method == "GET"){
      var aa = cc.path.split('?')[1];
      var bb = aa.split('=')[1];
      console.log("bb",bb);

      var body = JSON.stringify(chapterList[bb-1]);
      res.end(body);
      return;
    }    
      res.end(data);
